"""
Code to get your BRATS 2020 dataset ready for semantic segmentation. 
...
for img in range(len(t2_list)):   #Using t1_list as all lists are of same size
    ...
    if (1 - (counts[0]/counts.sum())) > 0.01:  #At least 1% useful volume with labels that are not 0
        ...
        np.save(Path('BraTS2020_TrainingData/input_data_3channels/images/image_'+str(img)+'.npy'), temp_combined_images)
        np.save(Path('BraTS2020_TrainingData/input_data_3channels/masks/mask_'+str(img)+'.npy'), temp_mask)
        

""" Code for splitting folder into train, test, and val.
Once the new folders are created rename them and arrange in the format below to be used for semantic segmentation using data generators. pip install split-folders
"""
